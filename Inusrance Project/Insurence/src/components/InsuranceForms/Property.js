...
function PropertyForm() {
  ...
  const handleCalculateSum = () => {
    ...
    const sum =
      ...
      (buildingAge >= 0 && buildingAge <= 5 ? totalPropertyValue * 0.04 : 0) +
      (buildingAge > 5 && buildingAge <= 10 ? totalPropertyValue * 0.05 : 0) +
      (buildingAge > 10 && buildingAge <= 15 ? totalPropertyValue * 0.06 : 0) +
      (buildingAge > 15 && buildingAge <= 20 ? totalPropertyValue * 0.07 : 0) +
      (buildingAge > 20 && buildingAge <= 25 ? totalPropertyValue * 0.08 : 0) +
      (buildingAge > 25 && buildingAge <= 30 ? totalPropertyValue * 0.09 : 0) +
  ...
  const validateInputs = () => {
    ...
    // validateField('duration', duration, 1);
      ...
      <div className='bg'>
        ...
        <div className='bg1'>
          ...
          <div className='form-container container'>
            ...
            {/* Render the Premium Prices and Calculate button */}
            {showPremiumPrices && (
              <>
                ...
                {!Object.values(errorMessages).some((errorMessage) => errorMessage) && (
                  <div className='d-flex'>
                    ...
                    <button  className='btn btn-primary mx-1'>{`1 years ${(totalSum + (totalSum * 0.3)).toFixed(2)}/yr`}</button>
                    <button  className='btn btn-primary mx-1'>{`2 years ${(totalSum + (totalSum * 0.25)).toFixed(2)}/yr`}</button>
                    <button  className='btn btn-primary mx-1'>{`3 years ${(totalSum + (totalSum * 0.2)).toFixed(2)}/yr`}</button>
                    <button  className='btn btn-primary mx-1'>{`4 years ${(totalSum + (totalSum * 0.15)).toFixed(2)}/yr`}</button>
                    <button  className='btn btn-primary mx-1'>{`5 years ${(totalSum + (totalSum * 0.1)).toFixed(2)}/yr`}</button>
                  </div>
                )}
              </>
            )}
